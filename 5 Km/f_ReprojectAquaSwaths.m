function f_ReprojectAquaSwaths(year,doy)
    ...
    system(sprintf('cp /bess19/Yulin/Data/Aqua_L2/%d/%03d/swaths.mat %s/Aqua_swaths.mat',year,doy,path));
  
    r = 0.05;
    [height,width] = size(mskLand);
    hm = importdata(sprintf('%s/Aqua_swaths.mat',path));

    ensemble = zeros(length(hm),height,width,'int16');
    ensemble(:) = 32767;
    for i = 1:length(hm)
        % Swath coordinates
        if ~exist(sprintf('/bess19/Yulin/BESSv2/Swath/MYD_SZA/%d%03d/MYD_SZA.%d%03d.%s.mat',year,doy,year,doy,hm{i}))
            continue;
        end
        ...
        data.msk = msk;
        data.pid = uint32(pid);
        save(sprintf('%s/Aqua_proj.%s.mat',path,hm{i}),'data');
        % Load solar zenith angle
        sza = importdata(sprintf('/bess19/Yulin/BESSv2/Swath/MYD_SZA/%d%03d/MYD_SZA.%d%03d.%s.mat',year,doy,year,doy,hm{i}));
        ensemble(i,msk) = sza(pid);        
    end
    % Identify min solar zenith angle for overlapped swaths
    [~,ind] = min(ensemble,[],1);
    data = squeeze(ind); save(sprintf('%s/Aqua_index.mat',path),'data');
